...
#GATHER INPUT KEYWORDS AND GROUP THEM INTO lists
	...
	def write_to_csv(self,path="keywordCSV.csv"):
		with open(path, "wb") as outfile:
			...
			w.writerow(list(self.keyword_groups.keys()))
			vals = list(self.keyword_groups.values())
			rows = list(*vals)
			for row in rows:
				w.writerow(row)
	...
	def make_keywords(self):
		for _ in range(len(self.input_keywords)):
	...
	def make_groups(self):
		kw_groups = []
		max_word = max(iter(self.word_frequencies.keys()), 
	...
	def get_input(self):
		...
		for line in x:
			...
			for y in range(1,len(w)+1):
				all_words += list(map(list, list(zip(*(w[i:] for i in range(y))))))
